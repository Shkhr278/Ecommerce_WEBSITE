...
export function ProductCard({ product, onViewDetails }) {
  ...
  const { data: favoriteData } = useQuery({
    queryKey: [product?.id ? `/api/favorites/${product.id}/check` : "/api/favorites"],
    enabled: !!product?.id,
  });
  ...
  const toggleFavoriteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [product?.id ? `/api/favorites/${product.id}/check` : ""] });
    },
  });
    ...
    <Card className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden fade-in">
        ...
        >
          <Heart
            className={cn(
              ...
              isFavorite ? "fill-red-500 text-red-500" : "text-neutral-500 hover:text-red-500"
            )}
          />
        </Button>
      </div>
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between mb-2">
          <Badge variant="secondary" className={cn("text-xs font-medium", getCategoryColor(product.category))}>
            {product.category}
          </Badge>
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            <span className="font-bold text-lg text-gray-900" data-testid={`text-product-price-${product.id}`}>
              {priceInfo.current}
            </span>
            {priceInfo.original && <span className="text-sm text-gray-500 line-through">{priceInfo.original}</span>}
          </div>
          <div className="text-xs text-neutral-500">{product.stockQuantity > 0 ? `${product.stockQuantity} in stock` : "Out of stock"}</div>
        </div>
        ...
        <div className="flex space-x-2">
          <Button variant="outline" className="flex-1" onClick={() => onViewDetails(product.id)} data-testid={`button-view-details-${product.id}`}>
            View Details
          </Button>
          <Button className="flex-1 bg-primary text-white hover:bg-primary/90" onClick={handleAddToCart} disabled={addToCartMutation.isLoading || product.stockQuantity === 0} data-testid={`button-add-to-cart-${product.id}`}>
